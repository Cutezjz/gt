...
class robot:
    ...
    def move(self, motion):  # Do not change the name of this function
        ...
        d = random.gauss(motion[1], self.distance_noise)  # distance traveled
        alpha = random.gauss(motion[0], self.steering_noise)  # steering angle

        beta = d / self.length * tan(alpha)  # turning angle

        if abs(beta) >= .001:
            R = d / beta  # turning radius.  it is infinite for straight motion
            ...
            result.x = cx + R * sin(self.orientation + beta)
            result.y = cy - R * cos(self.orientation + beta)
    ...
    def sense(self, *flag): #do not change the name of this function
        ...
        sigma = 0 if flag == 0 else self.bearing_noise
        sigma = 0  # test without noise
        for l in landmarks:
            angle = atan2(l[0] - self.y, l[1] - self.x) + random.gauss(0, sigma) - self.orientation
            angle %= 2.0 * pi
            Z.append(angle)
