...
p3 = []
s = sum(w)
normalized_weights = [i/s for i in w]
quantum_weight = min(normalized_weights)
for particle, weight in zip(p, normalized_weights):
    num = int(log10(weight/quantum_weight))
    print num
    # num = int(weight * N)
    for _ in range(num):
        p3.append(particle)
while len(p3) < len(p):
    p3.append(random.choice(p))
