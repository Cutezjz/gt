...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    print 'dim--->', dim
    dim_expanded = dim + 2
    print 'dim expanded--->', dim_expanded
    rowcol_mapping = [0, 1] + range(4, dim_expanded)
    print 'row col mapping--->', rowcol_mapping
    ...
    print 'omega before expand--->'
    Omega.show()
    print 'xi before expand--->'
    Xi.show()
    ...
    for k in range(len(data)):
        ...
        Omega = Omega.expand(dim_expanded, dim_expanded, rowcol_mapping, rowcol_mapping)
        print 'Omega after expand --->'
        Omega.show()
        Xi = Xi.expand(dim_expanded, 1, rowcol_mapping, [0])
        print 'Xi after expand --->'
        Xi.show()
        ...
        for i in range(len(measurement)):
            ...
            for b in range(2):
                ...
                Omega.value[n+b][m+b] += -1.0 / measurement_noise
                Omega.value[m+b][n+b] += -1.0 / measurement_noise
                print 'omega after measurement update--->'
                Omega.show()
                Xi.value[n+b][0]      += -measurement[i][1+b] / measurement_noise
                Xi.value[m+b][0]      +=  measurement[i][1+b] / measurement_noise
                print 'xi after measurement update--->'
                Xi.show()
        ...
        for b in range(2):
            ...
            Xi.value[n+b  ][0]        += -motion[b] / motion_noise
            Xi.value[n+b+2][0]        +=  motion[b] / motion_noise
        print 'omega after motion update--->'
        Omega.show()
        print 'xi after motion update--->'
        Xi.show()

        print 'omega right before take--->'
        Omega.show()

        print 'xi right before take--->'
        Xi.show()

        # apply take()
        A = Omega.take([0, 1], range(2, dim_expanded))
        print 'A--->'
        A.show()
        ...
        omega_prime = Omega.take(range(2, dim_expanded), range(2, dim_expanded))
        print 'omega prime--->'
        omega_prime.show()

        xi_prime = Xi.take(range(2, dim_expanded), [0])
        print 'xi prime--->'
        xi_prime.show()

        Omega = omega_prime - A.transpose() * B.inverse() * A
        Xi = xi_prime - A.transpose() * B.inverse() * C
        print 'omega when all is done--->'
        Omega.show()
        print 'xi when all is done---> '
        Xi.show()
